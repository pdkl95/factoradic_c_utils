...
echo "# <config>"
declare -p SKIP FACTORADIC GCC CLANG
...
bench_ruby() {
    bench 'for i in $(seq 1000) ; do factoradic -s "," "${FACTVALUE}" >/dev/null ; done'
}
...
do_bench_c() {
    ...
    bench 'for i in $(seq 1000) ; do ./fact2dec "${FACTVALUE}" >/dev/null ; done'
}
